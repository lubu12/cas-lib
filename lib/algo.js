...
// If trim is true, it will trim the string in the object
//
// Note: 
...
//
const removeEmptyKey = async (obj, findNest, trim)=> {
  removeEmptyKeySync(obj, findNest, trim);
  return Promise.resolve(obj);
}

const removeEmptyKeySync = (obj, findNest, trim)=> {
  for (var key in obj) {
    if (trim && typeof obj[key] === "string") { obj[key] = obj[key].trim(); }
    ...
    else if (findNest && Object.prototype.toString.call(obj[key]) === "[object Object]") { removeEmptyKeySync(obj[key], findNest); }
    else if (findNest && Object.prototype.toString.call(obj[key]) === "[object Array]") {
      for (let i = 0; i < obj[key].length; ++i) {
        if (trim && typeof obj[key][i] === "string") { obj[key][i] = obj[key][i].trim(); }
        if (obj[key][i] === "" || obj[key] == null || (typeof obj[key] === 'number' && isNaN(obj[key]))) { obj[key].splice(i, 1); }
        else { removeEmptyKeySync(obj[key][i], findNest); }
      }
    }
  }
  return obj;
