...
const { errorMsgFormatter } = require("./express-helper");
...
module.exports = class Logger extends MongoDBToolSet {
   ...
   * @returns {promise} Promise with the log id (to be used to log error)
   */
  static async log(urlPath, requestData, method, notifyError = true, rejectErr = false) {
    ...
    const result = await Logger.insertOne(Logger.collectionName, {
      ...
      url_path: urlPath,
      method,
      request: errorMsgFormatter(requestData)
    }, Logger.connString).catch(err => error = err);

    if (typeof error !== "undefined") {
      if (notifyError) { await Notifier.sendSystemErrorMsg("all", `Logger.log() | ${errorMsgFormatter(error)}`); }
      else { rejectErr = true; }
  ...
  static async error(id, statusCode, errorMsg, notifyError = true, rejectErr = false) {
    let error;
    await Logger.updateOne(Logger.collectionName, {
      $set: {
        status_code: statusCode,
        error: errorMsgFormatter(errorMsg)
      }
    }, { _id: MongoDBToolSet.getObjectId(id) }, Logger.connString).catch(err => error = err);

    if (typeof error !== "undefined") {
      if (notifyError) { await Notifier.sendSystemErrorMsg("all", `Logger.error() | ${errorMsgFormatter(error)}`); }
      else { rejectErr = true; }
