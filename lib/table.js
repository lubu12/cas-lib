...
class TableBase {
  /**
   * @class
   * @classdesc Table class with sort and pagination feature
   */
  constructor() {}
   ...
   * 
   * Note: default sort order is ascending if order is not provided at url parameter
   * 
   * @param {object} query Request url query
  ...
  getSortFilter(query) {
    let sorter = {
      [query.sort]: query.order === "desc" ? -1 : query.sort ? 1 : undefined,
      [query.sort1]: query.order1 === "desc" ? -1 : query.sort1 ? 1 : undefined,
      [query.sort2]: query.order2 === "desc" ? -1 : query.sort2 ? 1 : undefined,
    };

    return algo.removeEmptyKeySync(sorter);
  }
  ...
  getPagination(totalRow, query, originalUrl) {
    let pageNum = Number(query.page), rowPerPage = Number(query.num_per_page);
