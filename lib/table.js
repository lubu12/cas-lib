...
class TableBase {
   ...
   * 
   * Query names can be customized or it will be using the default values as follows:
   * ```
   * TableBase.queryName = {
   ...
   */
  getSortFilter(query) {
    if (Object.prototype.toString.call(TableBase.queryName.sorter) !== "[object Object]") {
      TableBase.queryName.sorter = {
        sort: "order",
        sort1: "order1",
        sort2: "order2"
      }
  ...
  getPagination(totalRow, query, originalUrl) {
    let pageNum = Number(query[TableBase.queryName.pageName ? TableBase.queryName.pageName : "page"]);
    let rowPerPage = Number(query[TableBase.queryName.numPerPageName ? TableBase.queryName.numPerPageName : "num_per_page"]);
